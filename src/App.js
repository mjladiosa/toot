...
const victorySound = React.useRef(null);
  ...
  useEffect(() => {
    ...
    try {
tootSound.current = new Audio("/tootfart.wav");
victorySound.current = new Audio("/tootlevel.mp3");
  ...
  const throwPopcorn = (e) => {
    ...
    const animate = () => {
      ...
      if (elapsed < duration) {
        ...
        const hitX = endX;
        const hitY = endY;
        const tootX = (tootPosition.x * gameArea.width) / 100;
        const tootY = (tootPosition.y * gameArea.height) / 100;
